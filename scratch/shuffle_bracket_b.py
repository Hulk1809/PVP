...
# Generate freshly shuffled matches for B only
matches_b = generate_bracket('bracket-b', part_b, randomize=True)

# Keep Bracket A and Bracket C intact
other_matches = {k: v for k, v in matches.items() if v.get('bracketId') != 'bracket-b'}

new_matches = {}
new_matches.update(other_matches)
new_matches.update(matches_b)

state['matches'] = new_matches
state['updatedAt'] = 1786998000000
...
with urllib.request.urlopen(push_req, timeout=6) as res:
    print("Pushed freshly shuffled Bracket B to EC2 status:", res.status)
